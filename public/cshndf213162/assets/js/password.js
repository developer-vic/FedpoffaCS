...
async function CheckForEmail(email) {
    ...
    const regCollectionRef = db.collection('projectHND23/cshndf213162/registration').doc(email);
    try {
        ...
        if (docSnapshot.exists) {
            ...
            if (data) {
                Swal.close();
                sessionStorage.setItem("reset_email_cshndf213162", email);
...
if (rpassword) {
    let userEmail = sessionStorage.getItem("reset_email_cshndf213162");
    ...
    rpassword.addEventListener('click', async function (e) {
        ...
        if (!password || !password2)
            ...
            showLoading();
            const db = firebase.firestore();
            const regCollectionRef = db.collection('projectHND23/cshndf213162/registration').doc(userEmail);
            try {
                const docSnapshot = await regCollectionRef.get();
                if (docSnapshot.exists) {
                    const data = docSnapshot.data();
                    if (data) {
                        ...
                        batch.commit().then(() => {
                            ...
                            sessionStorage.removeItem("reset_email_cshndf213162");  
                            sessionStorage.removeItem("current_user_cshndf213162");
                        }).catch((error) => {
                            // The write failed...
